package net.flashpunk.graphics 
{
	...
	public class Text extends Image
	{
		...
		/**
		 * The outline property to assign to new Text objects.
		 */
		public var outline:Boolean = false;
		
		/**
		 * The outline color to assign.
		 */
		...
		public function Text(text:String, x:Number = 0, y:Number = 0, options:Object = null, h:Number = 0)
		{
			...
			if (options)
			{
				...
				else
				{
					...
					if (options.hasOwnProperty("outline")) outline = options.outline;
					if (options.hasOwnProperty("outlineColor")) outlineColor = options.outlineColor;
					if (options.hasOwnProperty("embed")) embed = options.embed;
				}
			}
			
			_field.embedFonts = embed;
			_field.wordWrap = _wordWrap;
			if (outline)
				_field.filters = [new GlowFilter(outlineColor, 0.8, 2.0, 2.0, 10, 1)];
			_field.sharpness = 0;
			_field.antiAliasType = AntiAliasType.ADVANCED;
			_field.thickness = 100;
